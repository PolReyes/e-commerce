...
class ClienteController extends Controller
{
    ...
    public function postRecuperar(Request $request){
        ...
        if($cliente){
            ...
            $pwd = "CUR". mt_rand(10000,999999);

            $destino = $request->input("email");
            $nombre = "Cursania";
            $asunto =  "Esta es tu nueva contraseña para ingresar a cursania".
            $contenido = "Tu nueva contraseña autogenerada para ingresar a la aplicación es la siguiente: ". $pwd;
            $cabeceras = 'From: no-reply@cursania.com' . "\r\n" .
                //'Reply-To: webmaster@example.com' . "\r\n" .
                'X-Mailer: PHP/' . phpversion();
            mail($destino,$asunto,$contenido,$cabeceras);

            $cliente->password = Hash::make($pwd);

            if($cliente->save()){
                return response()->json([
                    "status" => 200,
                    "message" => "La nueva contraseña ha sido enviada a su direccion email",
                ],200);
            }
        }
